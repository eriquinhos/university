# Construa uma função que receba como entrada dois números reais a < b, um valor n ∈ N, o número intervalos no processo
# numérico, e uma função f(x) e retorne uma aproximação da integral entre a e b de f(x) usando a regra de Simpson
# repetida.
...
def simpson(a, b, n=3, f=lambda x: x):
    """
    Calcula uma aproximação da integral de f, calculada entre a e b, pela Regra de Simpson Repetida
    :param a: Limite inferior da integral definida;
    :param b: Limite superior da integral definida;
    :param n: Número de intervalos no processo numérico, com valor padrão n=3;
    :param f: Função matemática em x, com valor padrão f(x) = x;
    :return: Aproximação da integral de f, calculada entre a e b, pela Regra de Simpson;
    """
    ...
    qx = 0  # Primeiramente, definimos q(x) = f(x0) + 4f(x1) + 2f(x2) + 4f(x3) + 2f(x4) + ... + f(xn) = 0
    ...
    for j in range(0, len(fun)):  # Agora, vamos somar os valores de f(xi), i ∈ {0, 1, ..., n}, fazendo com que os
        ...
        if j == 0 or j == n:
            qx += fun[j]
        elif n > j > 0 and j % 2 == 0:
            qx += 2 * fun[j]
        else:
            qx += 4 * fun[j]

    integral = (h * qx)/3  # A aproximação da integral é dada por essa fórmula:
...
for v in n:
    print(f'\033[1;31m{simpson(a, b, v, f1)}')
    print(f'\033[1;32m{simpson(a, b, v, f2)}')
    print(f'\033[1;33m{simpson(a, b, v, f3)}')
    print(f'\033[1;34m{simpson(a, b, v, f4)}')
    print(f'\033[1;35m{simpson(a, b, v, f5)}')
    print(f'\033[1;36m{simpson(a, b, v, f6)}')
    print()
